...
TEST_GROUP(SnpesGatewayTests)
{
	...
	TimerItf_t MockTimer = { nullptr, mock_timer };
...
TEST(SnpesGatewayTests, Scan)
{
	...
	/* build a fake packet */
	build_signal(&(recv_buf), SCAN, 0xAA, 0xFF, 0x00, 0x00, 0x0, 0);
...
TEST(SnpesGatewayTests, SyncAndAck)
{
	...
	build_signal(&(recv_buf), SYNC, 0xAA, 0xFF, 0x55, 0x00, 0x1, 0);
	/* say that there is a packet availible */
	mock().expectOneCall("mock_avail").andReturnValue(1);
	mock().expectOneCall("mock_avail").andReturnValue(0);
	/* let the gateway compute */
	...
	build_signal(&(recv_buf), SYNC, 0xBB, 0xFF, 0x55, 0x00, 0x0, 0);
	/* say that there is a packet availible */
	mock().expectOneCall("mock_avail").andReturnValue(1);
	mock().expectOneCall("mock_avail").andReturnValue(0);
	/* let the gateway compute */
...
TEST(SnpesGatewayTests, FakeClientAndSyncFull)
{
	...
	snpes_gw_init(0x55, &TestLora, &TestTimer);
	/* fake client packet */
	build_signal(&(recv_buf), SYNC, 0xAA, 0x01, 0x55, 0x00, 0x0, 0);
	mock().expectOneCall("mock_avail").andReturnValue(1);
	mock().expectOneCall("mock_avail").andReturnValue(0);
	snpes_compute();
...
TEST(SnpesGatewayTests, Alive)
{
	...
	mock().expectNCalls(2, "mock_timer")
		.andReturnValue(((ALIVE_THLD * 1000) + 1000));
	snpes_compute();
	/* compute */
	mock().expectOneCall("mock_avail").andReturnValue(0);
	mock().expectNCalls(2, "mock_timer")
	...
	mock().expectNCalls(3, "mock_timer")
		.andReturnValue(((ALIVE_THLD * 1000) + 1000) + TIMEOUT_THLD);
	snpes_compute();
	/* compute */
	mock().expectOneCall("mock_avail").andReturnValue(0);
	mock().expectNCalls(2, "mock_timer")
...
TEST(SnpesGatewayTests, Transmission)
{
	...
	build_data(&recv_buf, 0xAA, 0x01, 0x55, 0x00, 0x1, &data,
		   sizeof(uint16_t));
	/* say that there is a packet availible */
	mock().expectOneCall("mock_avail").andReturnValue(1);
	mock().expectOneCall("mock_avail").andReturnValue(0);
...
TEST(SnpesGatewayTests, DataTimeout)
{
	...
	mock().expectOneCall("mock_avail").andReturnValue(0);
	mock().expectNCalls(2, "mock_timer")
		.andReturnValue(1 + TIMEOUT_THLD * 2);
	/* let the gateway compute */
	snpes_compute();
}
...
TEST(SnpesGatewayTests, TransmissionDataFull)
{
	...
	snpes_compute();
	/* build a fake ACK */
	build_signal(&(recv_buf), ACK, 0xBB, 0x02, 0x55, 0x00, 0x0, 0);
	/* say that there is a packet availible */
	mock().expectOneCall("mock_avail").andReturnValue(1);
	mock().expectOneCall("mock_avail").andReturnValue(0);
	/* let the gateway compute */
	...
	/* build a fake TRANS_START packet */
	build_signal(&(recv_buf), TRANS_START, 0xBB, 0x02, 0x55, 0x00, 0x4, 0);
	// TODO: get rid of this hack

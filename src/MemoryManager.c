...
void *memmgr_alloc(MemMgr_t *mem_ctx, uint16_t size)
{
	...
	if (ptr < (uint8_t *)mem_ctx->heap_end) {
		...
		if (size < GET_SIZE(ptr)) {
			...
			*(uint16_t *)(ptr + (GET_SIZE(ptr) - HEADER_SIZE)) =
				GET_SIZE(ptr) - size;
		}
...
void memmgr_free(MemMgr_t *mem_ctx, void *addr)
{
	...
	*(uint16_t *)(ptr + GET_SIZE(ptr) - HEADER_SIZE) = GET_SIZE(ptr);
	...
	if (ptr - FOOTER_SIZE > (uint8_t *)mem_ctx->heap_start &&
	    !IS_ALLOCATED((ptr - FOOTER_SIZE))) {
		/* update the new free chunk header */
		*(uint16_t *)(ptr - GET_SIZE((ptr - FOOTER_SIZE))) =
			GET_SIZE(ptr) + GET_SIZE(ptr - FOOTER_SIZE);
		/* update the new free chunk footer */
		*(uint16_t *)(ptr + (GET_SIZE(ptr) - HEADER_SIZE)) =
			GET_SIZE(ptr) + GET_SIZE(ptr - FOOTER_SIZE);
	...
	if (ptr + GET_SIZE(ptr) < (uint8_t *)mem_ctx->heap_end &&
	    !IS_ALLOCATED((ptr + GET_SIZE(ptr)))) {
		/* update the new free chunk footer */
		*(uint16_t *)(ptr + GET_SIZE(ptr) +
			      GET_SIZE(ptr + GET_SIZE(ptr)) - HEADER_SIZE) =
